...
package net.rewey.markscoins.keybind;

import org.lwjgl.glfw.GLFW;

import net.rewey.markscoins.procedures.BankOpenGUIProcedure;
...
public class OpenBankKeyBinding extends MarkscoinsModElements.ModElement {
	@OnlyIn(Dist.CLIENT)
	private KeyBinding keys;
	public OpenBankKeyBinding(MarkscoinsModElements instance) {
		super(instance, 54);
	...
	public void initElements() {
		keys = new KeyBinding("key.markscoins.open_bank", GLFW.GLFW_KEY_RIGHT_CONTROL, "key.categories.misc");
		...
		if (Minecraft.getInstance().currentScreen == null) {
			if (event.getKey() == keys.getKey().getKeyCode()) {
				if (event.getAction() == GLFW.GLFW_PRESS) {
					MarkscoinsMod.PACKET_HANDLER.sendToServer(new KeyBindingPressedMessage(0, 0));
		...
		if (type == 0) {
			{
				...
				BankOpenGUIProcedure.executeProcedure($_dependencies);
			}
		}
	}
}
